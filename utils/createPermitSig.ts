...
export const  createPermitSignature = async (permitData : PermitData) => {
    ...
    async function createPermit(spender: string, value: number, nonce: number, deadline: number) {
        const permit = { owner: permitData.currentAccount, spender, value, nonce, deadline }
        const Permit = [
      ...
      const permit = await createPermit(permitData.spenderAddress, permitData.tokensAmount, permitData.nonce, permitData.deadline);
